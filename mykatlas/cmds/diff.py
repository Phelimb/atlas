...
def run(parser, args):
	...
	mccortex_join = McCortexJoin(sample = args.sample, intersect_graph = args.graph, ingraph = args.ctx)
	intersect_graph_out_path = mccortex_join.run()
	## Remove the intersection from the original graph
	mccortex_subgraph = McCortexSubgraph(sample = args.sample, rmgraph = intersect_graph_out_path, ingraph = args.ctx, tmp_dir = mccortex_join.out_ctx_dir)
